...
if __name__ == '__main__':
    ...
    parser.add_argument('--local_dir', default='./data/openr1_math')
    ...
    # train_dataset = dataset['train'].select(range(7500))
    # import ipdb; ipdb.set_trace()
    train_dataset = dataset['train'].select(range(30000))
    ...
    def make_map_fn(split):
        ...
        def process_fn(example, idx):
            ...
            # random sample a int
            # if int == 1, print question and solution
            # import numpy as np
    ...
    # train_dataset = train_dataset.drop(columns=['solution', 'generations'])
    train_dataset = train_dataset.map(function=make_map_fn('train'), with_indices=True)
    train_dataset = train_dataset.remove_columns(["generations", "solution", "messages"])
